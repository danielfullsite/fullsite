...
export default function JevControlPanel() {
  ...
  const [showP19Import, setShowP19Import] = useState(false)
  const [p19Hash, setP19Hash] = useState('')
  const [p19Manifest, setP19Manifest] = useState('')
  ...
  function importP19(event: React.FormEvent) {
    event.preventDefault()
    try {
      const manifest = JSON.parse(p19Manifest) as unknown
      void send('/api/platform/jev', { source_sha256: p19Hash, p19_manifest: manifest }, 'import-p19', 'Manifiesto P19 importado fielmente como gate bloqueado.')
      setShowP19Import(false)
    } catch { setNotice('El manifiesto P19 no es JSON válido.') }
  }
