...
export async function deductIngredientsForOrder(
  ...
  orderId: string,
  actor: string,
): Promise<{ success: boolean; deductions: { ingredient: string; amount: number; unit: string; newStock: number }[]; alerts: string[]; resolution: { DB_MAPPING: string[]; FUZZY_FALLBACK: string[]; R1_OWNED: string[]; UNRESOLVED: string[] } }> {
  try {
  ...
  const alerts: string[] = []
  const resolution = { DB_MAPPING: [] as string[], FUZZY_FALLBACK: [] as string[], R1_OWNED: [] as string[], UNRESOLVED: [] as string[] }
  ...
  if (obs.total > 0) {
    ...
    console.info(
      `[deduct:summary] order=${orderId} items=${obs.total + obs.r1Skipped} r1=${obs.r1Skipped} ` +
  ...
  return { success: true, deductions, alerts, resolution }
    ...
    return { success: false, deductions: [], alerts: ['Error al descontar inventario'], resolution: { DB_MAPPING: [], FUZZY_FALLBACK: [], R1_OWNED: [], UNRESOLVED: [] } }
  }
}
