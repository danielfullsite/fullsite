  ...
  try {
    const r = await withRetry(
      () => uberFetch(`/v1/eats/stores/${storeId}/status`, {
        method: 'POST', scope: 'eats.store', body: JSON.stringify(body),
      }),
...
export async function activateStore(
  ...
  try {
    const r = await withRetry(
      () => uberFetch(`/v1/eats/stores/${storeId}/status`, { method: 'DELETE', scope: 'eats.store' }),
      { maxAttempts: 3, baseDelayMs: 500 }
    )
    const errText = r.ok ? undefined : await r.text()
    await auditLog({
...
export async function getStoreStatus(
  ...
  try {
    const r = await withRetry(
      () => uberFetch(`/v1/eats/stores/${storeId}/status`, { method: 'GET', scope: 'eats.store' }),
