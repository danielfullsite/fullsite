...
export async function getDashboardFromPosOrders(days: number = 30, clientId: string = getActiveClientSlug()): Promise<WansoftDaily[]> {
  ...
  const orders = await sbFetch('pos_orders',
    `select=mesa,mesero,personas,total,subtotal,iva,descuento,propina,metodo_pago,pagos,items,status,created_at&client_id=eq.${clientId}&status=in.(cerrada,cobrada)&created_at=gte.${cutoffStr}T00:00:00-06:00&order=created_at.asc&limit=5000`
