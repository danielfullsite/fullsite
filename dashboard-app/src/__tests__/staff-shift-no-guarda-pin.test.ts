...
describe('identidad del turno de personal', () => {
  ...
  it('persiste el UUID verificado del empleado y nunca el PIN crudo', () => {
    ...
    expect(source).toContain('autorizarOperacionConHuellaEnCaja()).staff')
    expect(source).not.toContain('ingresarConHuellaEnCaja')
    expect(source).toContain('autorizarOperacionConHuellaEnCaja')
    expect(source).not.toContain('ingresarConHuellaEnCaja')
