...
describe('capa 1 y 2 — el POS no deja cobrar dos veces desde la pantalla', () => {
  ...
  it('REGRESION: pregunta por `order_revision`, NUNCA por `updated_at`', () => {
    ...
    // `pos_orders` (`NEW.updated_at = NOW()`, incondicional), y el KDS escribe cada
    // vez que cocina palomea un platillo. Comparar esa marca bloqueaba el cobro de
    // una mesa porque alguien marcó un plato como listo — reproducido en AMALAY el
    // 2026-09-13. `order_revision` sólo lo mueven `r1_save_order` y `r1_add_items`
    // —dinero y renglones—, que es justo lo que esta guarda quiere detectar, y es
    // la misma revisión que usa el OCC del servidor.
    const check = cuerpoDe('app/pos/page.tsx', 'const checkOrderConflict = async', 2000)
