...
describe('El POS lo usa al anular', () => {
  ...
  it('Caja ofrece PIN o huella y entrega el mismo actor opaco con las disposiciones', () => {
    const pos = leer('src/app/pos/page.tsx')
    const i = pos.indexOf('function VoidOrderModal(')
    const modal = pos.slice(i, pos.indexOf('// ─── Cash Movement Modal', i))
    expect(modal).toMatch(/AutorizacionPinOHuella/)
    expect(modal).toMatch(/onPin=\{autorizarOperacionConPinEnCaja\}/)
    expect(modal).toMatch(/onHuella=\{autorizarOperacionConHuellaEnCaja\}/)
    expect(modal).toMatch(/onConfirmCaja\(reason, actor, disposiciones\)/)
  })
  it('transferir mesa en Caja también ofrece ambos medios y no trata el PIN como identidad', () => {
    const pos = leer('src/app/pos/page.tsx')
    const i = pos.indexOf('{mesaDestinoCaja !== null && (')
    const modal = pos.slice(i, pos.indexOf('{/* Void Order Modal', i))
    expect(modal).toMatch(/AutorizacionPinOHuella/)
    expect(modal).toMatch(/onPin=\{autorizarOperacionConPinEnCaja\}/)
    expect(modal).toMatch(/onHuella=\{autorizarOperacionConHuellaEnCaja\}/)
    expect(modal).not.toMatch(/moverMesaCaja\(pinInput\)/)
  })
})
