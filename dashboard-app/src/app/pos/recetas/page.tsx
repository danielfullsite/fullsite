      ...
      <div className="flex items-center gap-3 px-6 py-3 bg-slate-800/50 border-b border-slate-700">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full bg-slate-700 border border-slate-600 rounded-lg pl-10 pr-4 py-2.5 text-white placeholder-slate-400 text-sm focus:outline-none focus:border-amber-500"
          />
        </div>
        <button
          onClick={expandAll}
          className="px-3 py-2.5 bg-slate-700 hover:bg-slate-600 rounded-lg text-sm text-slate-300 transition-colors"
        >
                ...
                <div key={recipe.menu_item_id}>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-slate-400 text-sm">{recipe.ingredients.length} ingredientes</p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="font-semibold text-amber-400">{formatMXN(recipe.total_cost)}</p>
                      <p className="text-slate-500 text-xs">costo receta</p>
                    </div>
                  </button>
                        ...
                        <table className="w-full text-sm">
                            ...
                            {recipe.ingredients.map((ing, i) => {
                              ...
                              return (
                                <tr key={i} className="border-b border-slate-700/50 last:border-0">
                                  ...
                                  <td className="px-4 py-2.5 text-right text-slate-300">{ing.quantity}</td>
                                  <td className="px-4 py-2.5 text-slate-400">{ing.ingredient_unit}</td>
                                  <td className="px-4 py-2.5 text-right text-slate-400">{formatMXN(ingData?.cost_per_unit ?? 0)}</td>
                          ...
                          <tfoot>
                            <tr className="border-t border-slate-600">
                              <td colSpan={4} className="px-4 py-3 text-right font-semibold text-slate-300">Total receta</td>
