  ...
  const renderForm = (f: Promo, set: (v: Promo) => void, isNew: boolean) => (
    <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 mb-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <select value={f.type} onChange={e => set({ ...f, type: e.target.value as Promo['type'] })} className="border border-slate-200 rounded-lg px-3 py-2 text-sm">
          {Object.entries(TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
        </select>
        <input type="number" value={f.value || ''} onChange={e => set({ ...f, value: +e.target.value })} placeholder={f.type === 'percent' ? '% descuento' : '$ valor'} className="border border-slate-200 rounded-lg px-3 py-2 text-sm" />
        <input type="date" value={f.start_date} onChange={e => set({ ...f, start_date: e.target.value })} className="border border-slate-200 rounded-lg px-3 py-2 text-sm" />
        <input type="date" value={f.end_date} onChange={e => set({ ...f, end_date: e.target.value })} className="border border-slate-200 rounded-lg px-3 py-2 text-sm" />
        <input type="number" value={f.min_qty || ''} onChange={e => set({ ...f, min_qty: +e.target.value })} placeholder="Cant. minima" className="border border-slate-200 rounded-lg px-3 py-2 text-sm" />
      </div>
      {items.length > 0 && (
        <div className="mt-3">
          <p className="text-xs text-slate-500 mb-2">Aplica a ({f.applies_to.length} seleccionados):</p>
          <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto">
            {items.map(i => (
              <button key={i.id} onClick={() => toggleAppliesTo(f, set, i.id)}
                className={`px-2 py-1 rounded-lg text-xs font-medium transition-colors ${f.applies_to.includes(i.id) ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                {i.name}
              </button>
            ))}
          </div>
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {promos.map(p => (
              <tr key={p.id} className={`border-b border-slate-100 hover:bg-slate-50 ${!p.active ? 'opacity-40' : ''}`}>
                <td className="px-4 py-3 text-sm font-medium text-slate-900 flex items-center gap-2"><Tag size={14} className="text-slate-400" />{p.name}</td>
                ...
                <td className="px-4 py-3 text-xs text-slate-500">{p.applies_to?.length || 0} items</td>
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-1 justify-end">
                    <button onClick={() => setEditing(p)} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-500 flex items-center justify-center"><Pencil size={14}/></button>
        ...
        {promos.length === 0 && <p className="text-center text-slate-400 py-10 text-sm">No hay promociones. Crea la primera.</p>}
      </div>
