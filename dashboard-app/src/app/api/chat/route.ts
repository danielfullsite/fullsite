...
// ── OCM Fase 3 · el clon "sabe toda su info" ──
// Cuando un tenant NO tiene histórico en wansoft_daily (todos los clientes nuevos
// clonados del esqueleton), sintetizamos las filas diarias desde su pos_orders VIVO
// con el MISMO shape que wansoft_daily → todo el análisis del chat (día que más vende,
// meseros, platillos, métodos de pago, tendencias) funciona sin sembrar nada por tenant.
async function buildDailyFromOrders(
  sbUrl: string,
  ...
  const url = `${sbUrl}/rest/v1/pos_orders?client_id=eq.${encodeURIComponent(clientId)}`
    ...
    + `&select=created_at,total,subtotal,descuento,mesero,metodo_pago,personas,items`
  ...
  type Bucket = {
    ventas: number; brutas: number; descuentos: number; tickets: number; personas: number
  ...
  for (const o of orders) {
    ...
    if (!b) {
      b = { ventas: 0, brutas: 0, descuentos: 0, tickets: 0, personas: 0, meseros: {}, pagos: {}, platillos: {} }
      byDay.set(fecha, b)
    }
    ...
    for (const it of parseJsonb(o.items) as { nombre?: string; cantidad?: number; subtotal?: number; precio?: number }[]) {
