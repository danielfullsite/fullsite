...
export async function PATCH(request: NextRequest) {
  ...
  const existingRows = await sbGet<DeliveryOrderRow>(
    `delivery_orders?id=eq.${encodedId}&client_id=eq.${encodedClient}&select=id,client_id,platform,platform_order_id,status,customer_name,address,phone,total,payment_method,created_at,estimated_pickup,en_route_at,delivered_at,closed_at,items&limit=1`
  )
  ...
  if (existingOrder.platform === 'rappi' && existingOrder.platform_order_id && (body.status === 'preparando' || body.status === 'lista' || body.status === 'cancelada')) {
    const action = body.status === 'preparando' ? 'accept' : body.status === 'lista' ? 'ready' : 'cancel'
    const result = await syncRappiOrderAction({
      clientId: auth.clientId,
      platformOrderId: existingOrder.platform_order_id,
      action,
      reason: body.reason,
      minutesToReady: 20,
    ...
    if (!result.ok) {
      return Response.json({
        ok: false,
        error: 'RAPPI_PLATFORM_SYNC_FAILED',
        platform_sync: { attempted: true, ok: false, error: result },
      }, { status: 422 })
    }
    const updatedRows = await sbGet<DeliveryOrderRow>(
      `delivery_orders?id=eq.${encodedId}&client_id=eq.${encodedClient}&select=id,client_id,platform,platform_order_id,status,customer_name,address,phone,total,payment_method,created_at,estimated_pickup,en_route_at,delivered_at,closed_at,items&limit=1`
    )
    return Response.json({
      ok: true,
      order: updatedRows[0] ?? existingOrder,
      platform_sync: { attempted: true, ok: true },
    })
  }
