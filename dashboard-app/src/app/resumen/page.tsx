...
// Resumen — el dashboard del DUEÑO, rediseñado para ganarle a la competencia:
// números como héroe, agentes vivos con loop de feedback, y el sello offline.
// Reusa la MISMA capa de datos del home (getRecentDays/getLatestDay/agentes),
// sólo cambia el diseño. Datos reales, theme-aware (tokens del esqueleton).
// Regla esqueleton: costos/márgenes SÓLO aquí (dueño), NUNCA en el PDV operativo.

import { useEffect, useState, useCallback, useRef } from 'react'
import Link from 'next/link'
import { Wifi, TrendingUp, TrendingDown, Bot, Check, Clock, X, ArrowRight, AlertTriangle, Sparkles } from 'lucide-react'
...
export default function ResumenPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // Detecciones de los agentes (última por agente) + feedback guardado
      try {
        ...
        if (fb?.feedback) {
          ...
          for (const row of fb.feedback) m[row.agent_id] = row.action
          setActioned(m)
        }
      } catch {}
  ...
  useEffect(() => {
    ...
    on(); window.addEventListener('online', on); window.addEventListener('offline', on)
  ...
  const saveAction = (agentId: string, action: Action, det?: Detection) => {
    ...
    fetch('/api/agents/feedback', {
      ...
      body: JSON.stringify({ agent_id: agentId, action, insight_fecha: det?.fecha, insight_summary: det?.summary }),
    }).catch(() => {})
  }

  if (loading) {
    return <div className="grid place-items-center h-96"><div className="w-9 h-9 rounded-full border-2 animate-spin" style={{ borderColor: 'var(--accent)', borderTopColor: 'transparent' }} /></div>
  }
  ...
  // Delta vs promedio del mismo día de la semana (4 semanas)
  const sameDOWavg = (() => {
    if (!day) return 0
    ...
    return s.length ? s.reduce((a, d) => a + (Number(d.ventas_dia) || 0), 0) / s.length : 0
  })()
  const deltaPct = sameDOWavg > 0 ? ((ventas - sameDOWavg) / sameDOWavg) * 100 : null
  const dowName = day ? ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado'][new Date(day.fecha + 'T12:00:00').getDay()] : ''

  // Serie de ventas últimos 14 días
  const last14 = recent.slice(-14)
  const series = last14.map(d => Number(d.ventas_dia) || 0)
  const { line, area } = areaPath(series, 720, 150)

  // Top platillos y canales
  ...
  // La columna real es pago_metodos; el type la declara con acento. Aceptamos ambas.
  ...
  const canalTotal = canales.reduce((s, c) => s + (Number(c.total) || 0), 0) || 1
  ...
  return (
    <div className="max-w-[1180px] mx-auto w-full" style={{ color: 'var(--text-1)' }}>
      ...
      <div className="flex items-center gap-3 flex-wrap mb-5">
        ...
        <span className="text-sm font-semibold capitalize" style={{ color: 'var(--text-3)' }}>{dowName} · hoy</span>
      </div>
      ...
      <div className="grid gap-3.5 mb-3.5" style={{ gridTemplateColumns: 'minmax(0,1.1fr) minmax(0,1fr)' }}>
        ...
        <div className="rounded-2xl border p-5" style={{ background: 'var(--bento-card)', borderColor: 'var(--line)', boxShadow: 'var(--shadow-mid)' }}>
          <h2 className="text-sm font-bold">Hoy</h2>
          <div className="text-[42px] font-extrabold tabular-nums leading-none mt-2.5 mb-2 tracking-tight">{formatCurrency(ventas)}</div>
          {deltaPct !== null && (
            <span className="inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full"
              ...
              {deltaPct >= 0 ? '+' : ''}{deltaPct.toFixed(1)}% vs. {dowName} promedio
            </span>
          )}
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 mt-5">
            {[
              { l: 'Órdenes', n: formatNumber(tickets) },
              { l: 'Ticket promedio', n: formatCurrency(tp) },
              { l: 'Personas', n: formatNumber(personas) },
              { l: 'Propinas', n: formatCurrency(propinas) },
            ].map((k, i) => (
              <div key={i}>
                <div className="text-[11px] uppercase tracking-wide font-semibold" style={{ color: 'var(--text-3)' }}>{k.l}</div>
                <div className="text-lg font-bold tabular-nums mt-0.5">{k.n}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-2xl border p-5" style={{ background: 'var(--bento-card)', borderColor: 'var(--line)', boxShadow: 'var(--shadow-mid)' }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] font-bold tracking-wide inline-flex items-center gap-1.5" style={{ color: 'var(--accent-ink)' }}>
              <span className="w-2 h-2 rounded-full animate-pulse" style={{ background: 'var(--accent)' }} />EN VIVO
            </span>
          </div>
          {topDets.length === 0 ? (
            <div className="text-sm py-6 text-center" style={{ color: 'var(--text-3)' }}>Todo en orden — sin alertas de tus agentes.</div>
          ) : topDets.map(d => {
            const k = kindOf(d.priority)
            const act = actioned[d.agent_id]
            return (
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded-full flex-shrink-0"
                      style={{ color: KIND_COLOR[k], background: k === 'crit' ? 'var(--crit-soft)' : k === 'warn' ? 'var(--warn-soft, rgba(217,119,6,.12))' : 'var(--accent-soft)' }}>{KIND_LABEL[k]}</span>
                  </div>
                  <p className="text-xs mt-1 leading-snug" style={{ color: 'var(--text-2)' }}>{d.summary}</p>
                  <div className="flex gap-1.5 mt-2">
                    <button onClick={() => saveAction(d.agent_id, 'aplicado', d)} className="text-[11px] font-bold rounded-lg px-2.5 py-1 inline-flex items-center gap-1"
                      style={act === 'aplicado' ? { background: 'var(--accent)', color: '#fff' } : { background: 'var(--accent-soft)', color: 'var(--accent-ink)' }}>
                      <Check size={12} />{act === 'aplicado' ? 'Aplicado' : 'Aplicar'}
                    </button>
                    <button onClick={() => saveAction(d.agent_id, 'descartado', d)} className="text-[11px] font-bold rounded-lg px-2.5 py-1 border inline-flex items-center gap-1"
                      style={{ borderColor: 'var(--line)', color: 'var(--text-3)' }}><X size={12} />Descartar</button>
                  </div>
                </div>
              </div>
            )
          })}
          <Link href="/mission-control" className="inline-flex items-center gap-1.5 text-xs font-semibold mt-1" style={{ color: 'var(--accent-ink)' }}>
            Ver los 21 agentes <ArrowRight size={13} />
          </Link>
        </div>
      </div>
      ...
      <div className="rounded-2xl border p-5 mb-3.5" style={{ background: 'var(--bento-card)', borderColor: 'var(--line)', boxShadow: 'var(--shadow-mid)' }}>
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-sm font-bold">Ventas · últimos 14 días</h2>
          <span className="text-xs" style={{ color: 'var(--text-3)' }}>{last14.length} días</span>
        </div>
        {series.length >= 2 ? (
        ...
        <div className="rounded-2xl border p-5" style={{ background: 'var(--bento-card)', borderColor: 'var(--line)', boxShadow: 'var(--shadow-mid)' }}>
          ...
          {canales.length === 0 ? <p className="text-sm" style={{ color: 'var(--text-3)' }}>Sin datos aún.</p> : (
            <div className="space-y-2.5">
              {canales.sort((a, b) => b.total - a.total).slice(0, 5).map((c, i) => (
                <div key={i}>
                  <div className="flex justify-between text-xs mb-1"><span style={{ color: 'var(--text-2)' }}>{c.nombre}</span><span className="font-bold tabular-nums">{formatCurrency(c.total)}</span></div>
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--line)' }}>
                    <div className="h-full rounded-full" style={{ width: `${(c.total / canalTotal) * 100}%`, background: 'var(--accent)' }} />
                  </div>
                </div>
              ))}
            </div>
              ...
              <tbody>
                {topDishes.map((p, i) => (
                  <tr key={i} style={{ borderTop: i ? '1px solid var(--line)' : 'none' }}>
                    ...
                    <td className="py-2 pl-2 text-right font-bold tabular-nums">{formatCurrency(Number(p.total) || 0)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          )}
          <Link href="/rentabilidad" className="inline-flex items-center gap-1.5 text-xs font-semibold mt-3" style={{ color: 'var(--accent-ink)' }}>
            Ver margen por platillo <ArrowRight size={13} />
          </Link>
        </div>
      </div>
    </div>
