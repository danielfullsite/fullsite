      ...
      <header className="border-b border-[var(--line)] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--text-3)]">{DEMO_RESTAURANT.name} · 90 dias de historia</p>
          </div>
        </div>
      </header>
      ...
      <div className="p-6 max-w-[1400px] mx-auto space-y-6">
        ...
        <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={item.label} className="bg-[var(--surface)] border border-[var(--line)] rounded-xl p-4">
                <p className="text-xs text-[var(--text-3)] mb-1">{item.label}</p>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-xs text-[var(--text-4)]">vs {item.label === 'Ventas' ? formatDemoMXN(item.lastW) : item.lastW.toLocaleString()}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-5">
            ...
            <p className="text-xs text-[var(--text-3)] mt-1">Crecimiento mensual</p>
            <p className="text-xs text-[var(--text-4)] mt-0.5">{formatDemoMXN(sumLast30)} vs {formatDemoMXN(sumPrev30)}</p>
          </div>
          <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs text-[var(--text-3)] mt-1">Mejor dia (14d)</p>
            <p className="text-xs text-[var(--text-4)] mt-0.5">{best14.fecha}</p>
          </div>
          <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs text-[var(--text-3)] mt-1">Dia mas bajo (14d)</p>
            <p className="text-xs text-[var(--text-4)] mt-0.5">{worst14.fecha}</p>
          </div>
        </div>
        ...
        <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-5">
            ...
            {DEMO_HISTORY_14.map((d, i) => {
              ...
              return (
                <div key={d.fecha} className="flex-1 flex flex-col items-center gap-1" title={`${d.fecha}: ${formatDemoMXN(d.ventas_dia)}`}>
                  <div
                    className={`w-full rounded-t-md ${isToday ? 'bg-emerald-500' : isWeekend ? 'bg-purple-500/40' : 'bg-[var(--line)]'}`}
                    style={{ height: `${height}%` }}
                  />
          ...
          <div className="flex items-center gap-4 mt-2 text-xs text-[var(--text-4)]">
            <span className="flex items-center gap-1"><span className="w-2 h-2 bg-[var(--line)] rounded" /> L-V</span>
              ...
              return (
                <div key={m.month} className="bg-[var(--surface)] border border-[var(--line)] rounded-xl p-4">
                  ...
                  <div className="flex justify-between text-xs mt-1">
                    <span className="text-[var(--text-4)]">Prom/dia: {formatDemoMXN(m.promedioDia)}</span>
                    {mGrowth && (
