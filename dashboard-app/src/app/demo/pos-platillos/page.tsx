    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text-1)] p-6 md:p-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-3)]">{PLATILLOS.length} platillos &middot; Casa Montana</p>
          </div>
        </div>
        <button
          onClick={showToast}
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full pl-11 pr-4 py-3 bg-[var(--surface)] border border-[var(--line)] rounded-2xl text-sm text-[var(--text-1)] placeholder-zinc-600 focus:outline-none focus:border-white/10 transition-colors"
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map(p => (
          <div
            key={p.id}
            className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-5 flex flex-col justify-between gap-3 hover:border-white/10 transition-colors"
          >
            <div>
              ...
              <p className="text-xs text-[var(--text-3)] mt-1">{p.grupo}</p>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-emerald-400 font-bold text-lg">{formatDemoMXN(p.precio)}</span>
              <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
                p.activo ? 'bg-emerald-500/10 text-emerald-400' : 'bg-zinc-700/50 text-[var(--text-3)]'
              }`}>
