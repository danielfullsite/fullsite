    ...
    <div className="min-h-screen bg-[#0a0a0c] text-white p-6 md:p-10">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500">Opciones de tamano &middot; Casa Montana</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        {TAMANOS.map(t => (
          <div key={t.nombre} className="bg-white/[0.02] border border-white/5 rounded-2xl p-5 text-center">
            <div className="text-3xl font-bold text-cyan-400 mb-1">{t.label}</div>
            <div className="text-sm text-zinc-400">{t.nombre}</div>
          </div>
        ))}
      </div>
      ...
      {APLICADOS.map(grupo => (
        <div key={grupo.grupo} className="bg-white/[0.02] border border-white/5 rounded-2xl overflow-hidden mb-6">
          <div className="px-6 py-4 border-b border-white/5">
            <h3 className="font-semibold text-base">{grupo.grupo}</h3>
            <p className="text-xs text-zinc-500">Precio base: {formatDemoMXN(grupo.precioBase)}</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/5">
                  <th className="text-left px-6 py-3 text-xs text-zinc-500 font-medium">Producto</th>
                  {TAMANOS.map(t => (
                    <th key={t.nombre} className="text-center px-4 py-3 text-xs text-zinc-500 font-medium">{t.nombre}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={item} className="border-b border-white/[0.03] last:border-b-0">
                    ...
                    {TAMANOS.map(t => (
                      <td key={t.nombre} className="text-center px-4 py-3">
                        <span className={`text-sm font-semibold ${t.nombre === 'Grande' ? 'text-cyan-400' : t.nombre === 'Chico' ? 'text-zinc-500' : 'text-white'}`}>
