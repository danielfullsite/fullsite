...
const statusStyles = {
  ...
  expirada: 'bg-zinc-700/50 text-[var(--text-3)] border-zinc-600/30',
}
    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text-1)] p-6 md:p-10">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-[var(--text-3)]">{PROMOCIONES.length} promociones &middot; Casa Montana</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-4 text-center">
          ...
          <div className="text-xs text-[var(--text-3)] mt-1">Activas</div>
        </div>
        <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-4 text-center">
          <div className="text-2xl font-bold text-amber-400">{PROMOCIONES.filter(p => p.status === 'pausada').length}</div>
          <div className="text-xs text-[var(--text-3)] mt-1">Pausadas</div>
        </div>
        <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-4 text-center">
          <div className="text-2xl font-bold text-[var(--text-1)]">{PROMOCIONES.reduce((s, p) => s + p.usosMes, 0)}</div>
          <div className="text-xs text-[var(--text-3)] mt-1">Usos este mes</div>
        </div>
      </div>
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              <div className="flex-1">
                ...
                <p className="text-sm text-[var(--text-2)] mb-2">{p.descripcion}</p>
                <p className="text-xs text-[var(--text-4)]">Vigencia: {p.vigencia}</p>
              </div>
              <div className="text-right shrink-0">
                <div className="text-2xl font-bold">{p.usosMes}</div>
                <div className="text-[10px] text-[var(--text-3)]">usos este mes</div>
              </div>
            </div>
          </div>
        ))}
