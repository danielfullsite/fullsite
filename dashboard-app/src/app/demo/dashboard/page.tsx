...
// DS v1 card surface: subtle gradient ending in emerald tint + mid shadow
const CARD_STYLE = { background: 'var(--bento-card)', boxShadow: 'var(--shadow-mid)' } as const
// Signature dashboard-pro numeric look
    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text-1)]">
      ...
      <header className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid var(--line)' }}>
        ...
        <div className="flex items-center gap-3">
          <Link href="/demo/pos" className="flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-lg transition-opacity hover:opacity-90" style={{ background: 'linear-gradient(150deg, var(--accent-bright), var(--accent-deep))', color: '#04120c', boxShadow: '0 6px 18px rgba(16,185,129,0.28)' }}>
      ...
      <div className="p-6 max-w-[1400px] mx-auto space-y-6">
        {/* Prediction widget */}
        {projected > 0 && (
          <div className="relative overflow-hidden rounded-2xl border border-[var(--line)]" style={CARD_STYLE}>
            ...
            <div className="px-6 py-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs font-medium text-[var(--text-3)] uppercase" style={{ fontFamily: 'var(--font-mono)', letterSpacing: '0.12em' }}>Predicción de cierre</p>
                </div>
                <div className="flex items-center gap-1 text-xs text-[var(--text-3)]">
              ...
              <div className="flex items-end gap-4 mb-4">
                <p className="text-4xl font-bold tracking-tight tabular-nums" style={MONO}>{formatDemoMXN(projected)}</p>
                <div className="flex items-center gap-1 text-sm font-semibold mb-1 text-emerald-500">
                  <TrendingUp className="w-4 h-4" />
                  <span className="tabular-nums" style={MONO}>+{vsLastWeek.toFixed(1)}% vs semana pasada</span>
                </div>
              </div>
              <div className="mb-2">
                <div className="flex justify-between text-xs text-[var(--text-3)] mb-1.5">
                  <span>Progreso del día</span>
                  <span className="tabular-nums" style={MONO}>{pctDay.toFixed(0)}% completado</span>
                </div>
              ...
              <div className="grid grid-cols-3 gap-4 pt-3 border-t border-[var(--line)]">
                <div>
                  ...
                  <p className="text-sm font-semibold tabular-nums" style={MONO}>{formatDemoMXN(Math.max(0, projected - kpi.ventas_dia))}</p>
                </div>
                <div>
                  <p className="text-xs text-[var(--text-3)]">vs ayer</p>
                  <p className={`text-sm font-semibold tabular-nums ${vsYesterday >= 0 ? 'text-emerald-500' : 'text-rose-500'}`} style={MONO}>{vsYesterday >= 0 ? '+' : ''}{vsYesterday.toFixed(1)}%</p>
                </div>
                <div>
                  <p className="text-xs text-[var(--text-3)]">Meta: <span className="tabular-nums" style={MONO}>{formatDemoMXN(meta)}</span></p>
                  <p className={`text-sm font-semibold tabular-nums ${projected >= meta ? 'text-emerald-500' : 'text-amber-500'}`} style={MONO}>{((kpi.ventas_dia / meta) * 100).toFixed(0)}%</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {[
            ...
            <div key={card.label} className="border border-[var(--line)] rounded-2xl p-5" style={CARD_STYLE}>
              <div className="flex items-center justify-between mb-3">
                ...
                {card.change !== 0 && (
                  <span className={`text-xs font-medium tabular-nums ${card.change >= 0 ? 'text-emerald-400' : 'text-rose-400'}`} style={MONO}>
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          {[
            ...
            <div key={card.label} className="border border-[var(--line)] rounded-xl px-4 py-3" style={CARD_STYLE}>
              <div className="flex items-center gap-2 text-[var(--text-3)] text-xs mb-1">
                <card.icon size={12} />
                {card.label}
              </div>
              <p className="text-sm font-bold tabular-nums" style={MONO}>{card.value}</p>
            </div>
          ))}
        </div>
                ...
                return (
                  <div key={m.nombre} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-[var(--text-3)] mt-0.5"><span className="tabular-nums" style={MONO}>{m.tickets}</span> tickets · Propinas: <span className="tabular-nums" style={MONO}>{formatDemoMXN(m.propinas)}</span></p>
                    </div>
                  </div>
                )
              })}
            ...
            <div className="space-y-2">
              {DEMO_PLATILLOS.map((p, i) => (
                <div key={p.nombre} className="flex items-center justify-between py-1.5">
                  <div className="flex items-center gap-3">
                    <span className="text-[var(--text-4)] text-sm w-5 tabular-nums" style={MONO}>{i + 1}</span>
                    <span className="text-sm">{p.nombre}</span>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="text-xs text-[var(--text-3)]"><span className="tabular-nums" style={MONO}>{p.cantidad}</span> vendidos</span>
                    <span className="text-sm font-semibold w-20 text-right tabular-nums" style={MONO}>{formatDemoMXN(p.total)}</span>
                  </div>
                </div>
              ))}
            </div>
                ...
                return (
                  <div key={p.nombre}>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className="font-semibold tabular-nums" style={MONO}>{formatDemoMXN(p.total)}</span>
                    </div>
                    <div className="w-full h-2 bg-[var(--line-soft)] rounded-full">
                      <div className="h-2 bg-blue-500/50 rounded-full" style={{ width: `${pct}%` }} />
                    </div>
                    <p className="text-xs text-[var(--text-4)] mt-0.5 tabular-nums" style={MONO}>{pct.toFixed(1)}%</p>
                  </div>
                )
              })}
            </div>
            ...
            <div className="space-y-2">
              {DEMO_GRUPOS.slice(0, 8).map(g => (
                <div key={g.nombre} className="flex items-center justify-between py-1">
                  ...
                  <span className="text-sm font-semibold tabular-nums" style={MONO}>{formatDemoMXN(g.total)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="border border-[var(--line)] rounded-2xl p-5" style={CARD_STYLE}>
          ...
          <p className="text-xs text-[var(--text-3)] mb-4">Semana actual vs anterior: <span className={`font-bold tabular-nums ${DEMO_WOW.change.ventas >= 0 ? 'text-emerald-400' : 'text-rose-400'}`} style={MONO}>+{DEMO_WOW.change.ventas}%</span> (<span className="tabular-nums" style={MONO}>{formatDemoMXN(DEMO_WOW.thisWeek.ventas)}</span> vs <span className="tabular-nums" style={MONO}>{formatDemoMXN(DEMO_WOW.lastWeek.ventas)}</span>)</p>
        ...
        <div className="border border-[var(--line)] rounded-2xl p-5" style={CARD_STYLE}>
          ...
          <div className="grid grid-cols-3 gap-4">
            {DEMO_MONTHLY.map(m => (
              <div key={m.month} className="border border-[var(--line)] rounded-xl p-4" style={CARD_STYLE}>
                <p className="text-xs text-[var(--text-3)] mb-1">{m.month}</p>
                <p className="text-xl font-bold text-emerald-400 tabular-nums" style={MONO}>{formatDemoMXN(m.ventas)}</p>
                <div className="flex justify-between text-xs text-[var(--text-3)] mt-2">
                  <span className="tabular-nums" style={MONO}>{m.dias} días</span>
                  <span>TP: <span className="tabular-nums" style={MONO}>{formatDemoMXN(m.ticketPromedio)}</span></span>
                </div>
                <p className="text-xs text-[var(--text-4)] mt-1">Prom/día: <span className="tabular-nums" style={MONO}>{formatDemoMXN(m.promedioDia)}</span></p>
              </div>
            ))}
          </div>
        </div>
