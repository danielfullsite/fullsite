...
export default function DemoDashboard() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--demo-bg)', color: 'var(--demo-text)' }}>
      {/* Top bar */}
      <header className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid var(--demo-border)' }}>
        <div>
          <h1 className="text-lg font-bold">{DEMO_RESTAURANT.name}</h1>
          <p className="text-xs demo-text-2">{DEMO_RESTAURANT.location} · {DEMO_RESTAURANT.type}</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 text-xs demo-text-2">
      ...
      <div className="p-6 max-w-[1400px] mx-auto space-y-6">
        {/* Prediction widget */}
        {projected > 0 && (
          <div className="relative overflow-hidden rounded-2xl border demo-border bg-white/[0.02]">
            ...
            <div className="px-6 py-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs font-medium demo-text-2 uppercase tracking-wider">Predicción de cierre</p>
                </div>
                <div className="flex items-center gap-1 text-xs demo-text-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={card.label} className="demo-card rounded-2xl p-5">
              ...
              <p className="text-xs demo-text-2 mt-1">{card.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          {[
            ...
            <div key={card.label} className="demo-card rounded-xl px-4 py-3">
              <div className="flex items-center gap-2 demo-text-2 text-xs mb-1">
                <card.icon size={12} />
                {card.label}
              </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {/* Top meseros */}
          <div className="demo-card rounded-2xl p-5">
                ...
                return (
                  <div key={m.nombre} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="w-full h-1.5 demo-muted-bg rounded-full">
                        <div className="h-1.5 bg-emerald-500/60 rounded-full" style={{ width: `${pct}%` }} />
                      </div>
                      <p className="text-xs demo-text-2 mt-0.5">{m.tickets} tickets · Propinas: {formatDemoMXN(m.propinas)}</p>
                    </div>
                  </div>
                )
              })}
            ...
            <div className="space-y-2">
              {DEMO_PLATILLOS.map((p, i) => (
                <div key={p.nombre} className="flex items-center justify-between py-1.5">
                  <div className="flex items-center gap-3">
                    <span className="demo-text-3 text-sm w-5">{i + 1}</span>
                    <span className="text-sm">{p.nombre}</span>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="text-xs demo-text-2">{p.cantidad} vendidos</span>
                ...
                return (
                  <div key={p.nombre}>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="demo-text-2">{p.nombre}</span>
                      <span className="font-semibold">{formatDemoMXN(p.total)}</span>
                    </div>
                    <div className="w-full h-2 demo-muted-bg rounded-full">
                      <div className="h-2 bg-blue-500/50 rounded-full" style={{ width: `${pct}%` }} />
                    </div>
                    <p className="text-xs demo-text-3 mt-0.5">{pct.toFixed(1)}%</p>
                  </div>
                )
              })}
            </div>
            ...
            <div className="space-y-2">
              {DEMO_GRUPOS.slice(0, 8).map(g => (
                <div key={g.nombre} className="flex items-center justify-between py-1">
                  <span className="text-sm demo-text-2">{g.nombre}</span>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
            {DEMO_INSIGHTS.map((insight, i) => (
              <div key={i} className={`rounded-xl border p-4 ${
                ...
                <p className="text-xs demo-text-2 leading-relaxed">{insight.desc}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            {DEMO_HOURLY.map(h => {
              ...
              return (
                <div key={h.hora} className="flex-1 flex flex-col items-center gap-1" title={`${h.hora}: ${formatDemoMXN(h.ventas)}`}>
                  <div className={`w-full rounded-t-sm ${isCurrent ? 'bg-blue-500' : 'demo-muted-bg'}`} style={{ height: `${height}%` }} />
                  <span className="text-[8px] demo-text-3">{h.hora.slice(0, 2)}</span>
                </div>
              )
            })}
          </div>
        ...
        <div className="demo-card rounded-2xl p-5">
          ...
          <p className="text-xs demo-text-2 mb-4">Semana actual vs anterior: <span className={`font-bold ${DEMO_WOW.change.ventas >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>+{DEMO_WOW.change.ventas}%</span> ({formatDemoMXN(DEMO_WOW.thisWeek.ventas)} vs {formatDemoMXN(DEMO_WOW.lastWeek.ventas)})</p>
            ...
            {DEMO_HISTORY_14.map((d, i) => {
              ...
              return (
                <div key={d.fecha} className="flex-1 flex flex-col items-center gap-1" title={`${d.fecha}: ${formatDemoMXN(d.ventas_dia)}`}>
                  <div className={`w-full rounded-t-md ${isToday ? 'bg-emerald-500' : isWeekend ? 'bg-purple-500/40' : 'demo-muted-bg'}`} style={{ height: `${height}%` }} />
                  <span className="text-[9px] demo-text-3">{d.fecha.slice(8)}</span>
                </div>
              )
            })}
          </div>
          <div className="flex items-center gap-4 mt-2 text-xs demo-text-3">
            <span className="flex items-center gap-1"><span className="w-2 h-2 demo-muted-bg rounded" /> L-V</span>
        ...
        <div className="demo-card rounded-2xl p-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            {DEMO_MONTHLY.map(m => (
              <div key={m.month} className="demo-card rounded-xl p-4">
                <p className="text-xs demo-text-2 mb-1">{m.month}</p>
                <p className="text-xl font-bold text-emerald-400">{formatDemoMXN(m.ventas)}</p>
                <div className="flex justify-between text-xs demo-text-2 mt-2">
                  <span>{m.dias} días</span>
                  <span>TP: {formatDemoMXN(m.ticketPromedio)}</span>
                </div>
                <p className="text-xs demo-text-3 mt-1">Prom/día: {formatDemoMXN(m.promedioDia)}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="text-center py-6">
          <p className="text-xs demo-text-3">Powered by <strong>fullsite</strong> — IA operativa para restaurantes</p>
