...
interface CostItem {
  ...
  sospechoso?: {            // probable capture error in fuente histórica recipe
    ingrediente: string
    costo: number
    ...
    async function load() {
      try {
        ...
        if (api && Array.isArray(api.recipes) && api.recipes.length > 0) {
          ...
          // Modificadores reales (wsm-*): las ~93 recetas huérfanas de fuente histórica
          ...
          for (const recipe of recipes) {
            ...
            for (const ing of ingredients) {
              ...
              if (qty > 0 && budgetCost > 0) {
                costoTotal += budgetCost  // NOT qty * budgetCost — fuente histórica already calculates the total
                validIngredients++
            ...
            // en fuente histórica (unidad/cantidad mal, ej. MOZARELA en litros a $187/porción)
            const sospechoso =
