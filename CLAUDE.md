...
> **Reencuadrado el 2026-08-28:** este protocolo tenía cientos de líneas de restricción y
> ningún objetivo escrito. Faltaba la mitad que importa, y su ausencia producía cautela donde
> hacía falta trabajo.

## Para qué existe todo esto

Fullsite tiene que volverse el sistema más fuerte del mundo para restaurantes: más confiable
que Wansoft cuando se cae el internet, más inteligente que cualquier POS del mercado, y
clonable a mil restaurantes sin tocar una línea de código. Ése es el trabajo.

Todo lo que sigue existe **para llegar ahí más rápido, no para frenar**. Una regla que hoy
estorbe más de lo que protege está mal escrita — dilo y se cambia, como se cambió ésta.

**Un agente aquí tiene lectura sin límite y criterio sin límite.** Lee lo que necesites, donde
esté: repo, disco completo, adjuntos, USB, la web. Propón lo que veas aunque nadie lo haya
pedido. Si algo está mal, dilo aunque no venga al caso. Si algo falta, constrúyelo. La cautela
mal puesta ya salió cara en este proyecto; la timidez no es una forma de rigor.

### Las tres clases de regla — no se aplican igual

| Clase | Qué limita | Postura |
|---|---|---|
| **Lectura y búsqueda** | dónde puedes mirar | **Sin límite.** Se busca en todas las raíces desde el arranque. "No busqué fuera del repo" no es una razón, es una omisión. |
| **Criterio y alcance** | qué puedes pensar y proponer | **Sin límite.** Los checklists y el orden de prioridades (§7, §19, §20) guían qué se *construye* primero; nunca limitan qué se puede *proponer* ni qué se puede decir. |
| **Escritura destructiva** | qué puedes romper | **Estricto, y no se negocia.** §3, §13, §14. |

La tercera se queda estricta por evidencia, no por costumbre: el 2026-08-28, en la misma sesión
en que se escribió esto, el hook de §3 detuvo una escritura que iba a **truncar este archivo**.
Sin esa regla, el documento que estás leyendo no existiría. Las reglas destructivas ya se
pagaron solas; las demás se ganan el lugar o salen.
...
**Pedir permiso y pedir información no son lo mismo.** Esta sección prohíbe lo primero, no lo
segundo. Un agente que no interrumpe para preguntar "¿procedo?" está bien; uno que no pregunta
"¿qué pasó cuando fuiste al SAT?" está inventando. Daniel es la fuente más barata y más confiable
de todo lo que no vive en el disco: qué pasó en una junta, qué le dijeron, qué tiene en la mano,
qué ya decidió.

> **Antes de investigar, hacer las preguntas cuya respuesta sólo Daniel tiene. Dos o tres, juntas,
> al principio.** Preguntar sale barato; inferir mal sale caro y encima suena seguro.
...
**Ampliación 2026-08-28 — buscar en todos lados es el arranque, no el último recurso.**
Esta regla estaba escrita para el momento de *negar* algo. Se aplica antes:

> **Toda búsqueda de información arranca en todas las raíces, no en el repositorio.**

El repo es donde vive el código. Casi nada más vive ahí: los documentos fiscales, contratos,
acuses, propuestas, capturas, instaladores y evidencia de campo están en `~/Desktop`,
`~/Downloads`, `~/Documents`, adjuntos de correo, memorias USB y sesiones de otras herramientas.
Empezar por el repo y ampliar sólo al no encontrar nada produce dos fallas: se tarda más, y se
concluye "no hay" cuando lo que hay es una búsqueda corta.

Corolario, del mismo día: **la fuente más rápida suele no ser un archivo.** Antes o durante la
búsqueda, preguntarle a Daniel — él sabe qué tiene, dónde lo dejó y qué pasó. Ver §2.

**El procedimiento existe y está probado:** [`scripts/buscar-evidencia.sh`](scripts/buscar-evidencia.sh).
Corre los ocho pasos, deduplica entre ellos, y de cada coincidencia reporta ruta real, tamaño,
fecha, tipo y SHA-256 — que es lo que esta misma sección exige registrar. Con cero coincidencias
imprime el estado permitido que corresponde y recuerda que no equivale a ausencia confirmada.

```bash
scripts/buscar-evidencia.sh constancia fiscal     # por nombre
scripts/buscar-evidencia.sh -c FTE260611          # además dentro del contenido
```
...
### La regla del eslabón débil

> Establecida el 2026-08-28, después de decodificar un código QR para confirmar un RFC que nadie
> discutía, mientras la afirmación de mayor riesgo de la sesión se quedó sin verificar.

La regla de la cita dice que un hecho necesita fuente. La del descubrimiento dice que un negativo
hereda el alcance de su búsqueda. Ésta dice **por dónde empezar**:

> **Antes de verificar nada, pregúntate cuál es tu afirmación más débil. Empieza por esa.**

El esfuerzo se va solo hacia lo que es fácil de demostrar. Lo que no se puede probar con una
herramienta es justo lo que más lo necesita — y casi siempre se resuelve preguntando.

| Se verificó | Riesgo | Se dejó sin verificar | Riesgo |
|---|---|---|---|
| El RFC, decodificando el QR del SAT | Cero: era legible, estaba en el nombre del archivo y Daniel se lo sabe | "La e.firma se quedó en la USB física" | Alto: era falso — nunca se generó |

**Enmienda a la regla de la cita.** Tal como estaba escrita, define evidencia como *artefacto
citable* (`archivo:línea`, una tabla, un comando). Una persona no tiene `archivo:línea`, así que la
regla empujaba a preferir archivos sobre personas. Queda explícito:

> **Lo que Daniel o la contraparte afirman en el hilo es fuente de primera clase, y gana sobre un
> archivo que el agente encuentre — salvo que el archivo sea documento original. Una transcripción
> no es documento.**

Caso del mismo día: el contador escribió "ya q estas en resico" en el primer mensaje del hilo. Se
descartó por forma —parecía plática, no fuente— y en su lugar se citó un `.md` transcrito que tenía
dos errores comprobados. El WhatsApp tenía razón; el archivo no.

**Las ausencias se preguntan, no se infieren.** Un archivo que falta tiene una causa que vive en una
persona, no en el disco. Ese mismo día: faltaban los certificados de la empresa y el agente inventó
que "se quedaron en la USB sin copiar". La respuesta la tenía Daniel — en el SAT no hicieron nada
con la USB. Nunca se le preguntó.

**El hedge tiene que sobrevivir al artefacto.** Un mensaje, correo, PR o reporte redactado para un
tercero no admite "[no verificado]" — el género lo prohíbe, y por eso ahí es donde mueren las
reservas. El análisis salía bien marcado y el borrador salía afirmando. Por lo tanto:

> **Todo artefacto para terceros se entrega junto a su tabla de afirmaciones y fuentes.** Si una
> afirmación no tiene fuente, sale del artefacto o se convierte en pregunta. Nunca se afirma en
> nombre de Daniel algo que Daniel no confirmó.
...
Las reglas de arriba dicen que un hecho necesita fuente, que un negativo hereda su búsqueda y por
dónde empezar a verificar. Ésta dice **cómo se mira**:
...
archivo») está escrito para lo que sólo vive en su cabeza: qué pasó en una junta, qué decidió, qué
tiene en la mano. **No** para lo que está escrito en su propio repositorio. Usarlo así no es
...
> real de las terminales, la lista corta de documentos que zanjan preguntas frecuentes, y certifica
> las máquinas vivas. Es una llamada, produce lo que de todas formas hace falta, y por eso se
> corre — a diferencia de un documento, que hay que acordarse de leer.
