...
| `printers_config` hosts `127.0.0.1:19100-19104`, 5 named printers (`twin-cocina-1/2`, `twin-barra`, `twin-entrada`, `twin-caja`) | synthetic twin overrides of KNOWN topology | Harness mock listeners standing in for the KNOWN physical devices (Jul-12 printers.json): cocina = ARRAY of 2 (fría .21 + caliente .40), barra .30, PDV3 usb TICKET → `twin-entrada` (TCP in twin), SERVER1 usb PANADERIA/EC01 → `twin-caja` (TCP in twin, also carries `caja`+`tickets` default). Per-terminal ticket routing via `terminals[].ticket_station` (`tickets-entrada`/`tickets-caja` are twin station ids — station ids are free-form in printer-config-schema v2). ESCONDITE/PDV1 has NO working local ticket printer (KNOWN FAIL, FIELD-NOTES:105-112) — fixture routes its tickets to SERVER1's `tickets-caja` as the represented workaround. |
| `terminals[].alias` (ENTRADA/ESCONDITE/CAJA) + `ticket_station` + `local_ticket_printer:false` on PDV1 | derived/synthetic | Aliases are the KNOWN terminal names; `ticket_station` encodes the KNOWN local-printer-per-terminal behavior; the PDV1 flag encodes the KNOWN broken-printer state. Field names themselves are twin fixture extensions (not config-schema.js fields). |
| `users[]` entry `Revocado Twin` (`active:false`) | synthetic | Exists only so the revoked-staff-offline probe has a fixture-marked revoked staff. Revocation enforcement is NOT a Bridge/protocol feature (owner: web app UI pos-manager-auth + Supabase pos_staff). |
