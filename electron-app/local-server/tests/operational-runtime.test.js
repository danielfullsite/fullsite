...
test('opening accounts requires financial revision for additions and preserves preparation independently from payment', async t => {
  ...
  for (const type of ['ORDER_MOVE', 'ORDER_VOID']) {
    const patch = type === 'ORDER_VOID' ? { reason: 'Cancelar prueba', inventory_dispositions: [{ line_id: 'line-coffee', disposition: 'return_stock' }] } : {}
    assert.equal((await s.send(type, saveFields(s, { expected_revision: 2, ...patch }))).code, 'FINANCIAL_ORDER_LOCKED')
  }
...
test('authorized move releases old table; cancellation is durable and cannot be resurrected by cloud polling', async t => {
  ...
  assert.equal((await s.send('ORDER_VOID', orderFields(3, { reason: 'Cliente se retira', inventory_dispositions }), { actor: { ...actor, permissions: permissionsFor('gerente') } })).code, 'PERMISSION_DENIED')
  assert.ok((await s.send('ORDER_VOID', orderFields(3, { reason: 'Cliente se retira', inventory_dispositions }))).event)
  assert.equal(s.state.toSnapshot().salon_orders.length, 0)
  assert.equal(s.state.toSnapshot().kds_orders.length, 0)
  s = await s.restart()
  ...
  assert.deepEqual(JSON.parse(s.state.getOrder('mother').items).map(item => ({ id: item.id, cancelled: item.cancelled, disposition: item.inventory_disposition })),
    [{ id: 'line-coffee', cancelled: true, disposition: 'retain_consumption' }])
...
test('full cancellation requires one trusted disposition for every canonical line', async t => {
  const s = await setup(t)
  await s.send('ORDER_SAVE', saveFields(s))
  const base = orderFields(1, { reason: 'Error de captura' })
  assert.equal((await s.send('ORDER_VOID', base)).code, 'CANCELLATION_DISPOSITION_REQUIRED')
  assert.equal((await s.send('ORDER_VOID', { ...base, inventory_dispositions: [] })).code, 'CANCELLATION_DISPOSITION_REQUIRED')
  assert.equal((await s.send('ORDER_VOID', { ...base, inventory_dispositions: [{ line_id: 'forged', disposition: 'return_stock' }] })).code, 'CANCELLATION_DISPOSITION_REQUIRED')
  assert.equal((await s.send('ORDER_VOID', { ...base, inventory_dispositions: [{ line_id: 'line-coffee', disposition: 'invented' }] })).code, 'INVALID_CANCELLATION_DISPOSITION')
  assert.equal((await s.send('ORDER_VOID', { ...base, inventory_dispositions: [{ line_id: 'line-coffee', disposition: 'return_stock', quantity: 99 }] })).code, 'INVALID_CANCELLATION_DISPOSITION')
  const accepted = await s.send('ORDER_VOID', { ...base, inventory_dispositions: [{ line_id: 'line-coffee', disposition: 'return_stock' }] })
  assert.ok(accepted.event)
  assert.equal(JSON.parse(accepted.result.operational_order.items)[0].inventory_disposition, 'return_stock')
})
...
test('Eduardo: shared order numbers survive retries/restart and restart at one after Z without erasing history', async t => {
  ...
  await s.send('ORDER_VOID', orderFields(2, { reason: 'Cancelar prueba', inventory_dispositions: [{ line_id: 'line-coffee', disposition: 'return_stock' }] }))
  await s.send('ORDER_VOID', { order_id: 'second', turno_id: 't1', expected_revision: 1, reason: 'Cancelar prueba', inventory_dispositions: [{ line_id: 'line-coffee', disposition: 'return_stock' }] })
...
test('Order numbers serialize competing terminals, reject without consuming, and survive an empty salon snapshot', async t => {
  ...
  for (let n = 1; n <= 3; n++) assert.ok((await s.send('ORDER_VOID', { order_id: `parallel-${n}`, turno_id: 't1', expected_revision: 1, reason: 'Cancelación laboratorio', inventory_dispositions: [{ line_id: 'line-coffee', disposition: 'return_stock' }] })).result)
