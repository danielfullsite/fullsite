...
test('the C# service fails closed before capture or template mutation and rejects replay', () => {
  ...
  const configLoader = source.slice(source.indexOf('static string[] UserDataCandidates()'), source.indexOf('static bool ConstantTimeEquals'))
  assert.ok(configLoader.indexOf('FULLSITE_USER_DATA_DIR') < configLoader.indexOf('@"C:\\fullsite\\config.json"'))
  assert.match(configLoader, /Path\.Combine\(roaming, "fullsite-pos"\)/)
  assert.match(configLoader, /config\.json ignorado \(incompleto\)/)
  assert.doesNotMatch(configLoader, /Console\.WriteLine\([^\n]*(supabaseKey|ipcSecret)/)
})
...
test('Electron provisions the secret before probing/spawning and authenticates an existing service', () => {
  ...
  assert.match(start, /FULLSITE_USER_DATA_DIR:\s*app\.getPath\('userData'\)/)
  ...
  const updater = main.slice(main.indexOf('function fingerprintFileHash'), main.indexOf('function startFingerprintService'))
  assert.match(updater, /createHash\('sha256'\)/)
  assert.match(updater, /Get-CimInstance Win32_Process/)
  assert.match(updater, /ExecutablePath/)
  assert.match(updater, /Stop-Process -Id \$_\.ProcessId/)
  assert.match(updater, /fingerprint-backup-/)
  assert.match(updater, /fs\.renameSync/)
})
...
test('the manual repair path provisions the same private secret and verifies mutual HMAC', () => {
  ...
  assert.match(install, /Join-Path \$env:APPDATA 'fullsite-pos'/)
  assert.match(install, /Find-FullsiteUserData/)
  assert.doesNotMatch(install, /\$config\.supabaseUrl|\$config\.supabaseAnonKey/)
  assert.match(install, /WindowsIdentity\]::GetCurrent\(\)\.Name/)
  assert.match(install, /Get-FileHash -LiteralPath \$path -Algorithm SHA256/)
  assert.match(install, /Get-CimInstance Win32_Process/)
  assert.match(install, /ExecutablePath/)
  assert.match(install, /fingerprint-backup-/)
  assert.doesNotMatch(install, /Filter '\*bridge\*\.bat'/)
  ...
  assert.match(docs, /%APPDATA%\\fullsite-pos\\fingerprint/)
})
