...
async function main() {
  ...
  const emptied = JSON.parse(sql(call('last','src','i2',7)))
  assert.equal(emptied.source_order.items.length,0)
  assert.equal(emptied.source_order.status,'cancelada')
  assert.equal(emptied.source_order.total,0)
  assert.notEqual(emptied.source_order.payment_status,'pagada')
  assert.match(emptied.source_order.notas,/transferida a/)
  assert.equal(emptied.target_order.items.length,2)
  assert.deepEqual(JSON.parse(sql(call('last','src','i2',7))),emptied)
  console.log('PASS last item transfer frees empty source without creating a payment and replays its canonical receipt')
  reset()
  const insert = `insert into pos_orders(id,client_id,location_id,turno_id,mesa,items,subtotal,iva,total,descuento) values('ordinary','test','loc','turn',7,'[]',0,0,0,0);`
  const race = await Promise.all([concurrent(call('transfer-vs-save','src','i1',7)),concurrent(insert)])
  // Either insert wins and transfer adopts it, or transfer creates and ordinary
  // insert is rejected by the same unique index all writers obey.
  assert(race.some(result => result.code === 0))
  for (const failed of race.filter(result => result.code !== 0)) assert.match(failed.error,/pos_orders_una_cuenta_activa_por_mesa/)
  if (race[0].code !== 0) assert.equal(sql("select count(*) from pos_transfer_operations where operation_id='transfer-vs-save'"),'0')
  assert.equal(sql("select count(*) from pos_orders where client_id='test' and location_id='loc' and mesa=7 and status in ('abierta','enviada','preparando','lista') and coalesce(payment_status,'pendiente')<>'pagada'"),'1')
  assert.equal(sql("select count(*) from pos_orders o cross join lateral jsonb_array_elements(o.items) x where x->>'id'='i1'"),'1')
  console.log('PASS concurrent ordinary creation and transfer cannot create two active accounts on the same table')
  reset()
  sql("update pos_orders set payment_status='pagada',pagos='[{\"monto\":23.2}]' where id='dst'")
  const reused = JSON.parse(sql(call('paid-kitchen')))
  assert.notEqual(reused.target_order_id,'dst')
  assert.equal(sql("select status from pos_orders where id='dst'"),'abierta')
  assert.equal(sql("select count(*) from pos_orders where mesa=2"),'2')
  console.log('PASS paid account may keep kitchen history while table receives a new unpaid account')
}
