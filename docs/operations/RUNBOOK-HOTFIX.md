...
Si el bypass por PR no alcanza —por ejemplo hay que reescribir historia— es preferible
**quitar y restaurar** la protección de forma explícita antes que dejar `enforce_admins`
permisivo sin control:
...
- **`test` sólo corre con cambios en `dashboard-app/**`.** Un PR de docs, workflows o
  `electron-app/` no recibe el check y queda bloqueado — es el caso 3 de arriba. La solución
  de fondo es quitar el filtro `paths` de `test-dashboard.yml` para que `test` siempre
  reporte. Mientras no se haga, este runbook es el desvío.
- El bypass no está auditado automáticamente. El registro de arriba es manual y depende de
  la disciplina de quien lo usa.
