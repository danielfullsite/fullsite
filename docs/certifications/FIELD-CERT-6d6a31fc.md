...
> **Pin:** `6d6a31fcabfc189ab8fd05fb0f6c0936fcbe916d`
...
> **Procedencia del pin — leer antes de usar la línea base.**
> Este documento es `FIELD-CERT-418933f4.md` re-pineado el **2026-09-20**. Cambiaron
> cinco líneas y nada más: el pin, `PROD_SHA`, `VERCEL_DEPLOYMENT_ID`, el pre-flight P3
> y el `TARGET_SHA` del cierre. F1–F9, las condiciones de aborto, los oráculos y las
> advertencias de KDS/impresión/offline están **sin tocar**.
>
> La línea base de §1 y las cuatro huellas de §0 **se heredaron, y se revalidaron
> contra producción el 2026-09-20**: Δ=0 en las 15 métricas y 4/4 huellas idénticas.
> No son números viejos copiados a ciegas — se volvieron a medir.
>
> El delta `418933f4 → 6d6a31fc` es un solo commit (#430) que toca únicamente el
> webhook de Uber Eats: cero cambios en POS, KDS, impresión, caja, corte y offline,
> es decir cero cambios en el camino que F1–F9 ejercen. Por eso el re-pin no invalida
> la línea base. Si el próximo delta **sí** toca ese camino, esto no aplica y la base
> se vuelve a congelar desde cero.
...
| | |
|---|---|
| `PROD_SHA` | `6d6a31fcabfc189ab8fd05fb0f6c0936fcbe916d` |
| `VERCEL_DEPLOYMENT_ID` | `dpl_BTZdRVvp6F4ToR3McJdzaWiwkUkF` |
...
| P3 | La terminal sirve `6d6a31fc` | en la caja: DevTools → Network → cualquier `/_next/` | sirve otro build |
...
```
FIELD_CERT_RESULT      =
TARGET_SHA             = 6d6a31fcabfc189ab8fd05fb0f6c0936fcbe916d
PASOS                  = x/9
