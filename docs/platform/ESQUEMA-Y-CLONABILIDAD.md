...
- **No cubre `auth`, `storage` ni otros esquemas de Supabase.** Sólo `public` y
  `private`.
- **El workflow no se ha ejecutado nunca**, porque faltan los secretos. Está *implementado
  y probado en sus partes* (el script tiene sus tres casos verificados, el YAML pasa
  `actionlint` con cero hallazgos), **no validado de punta a punta**.
