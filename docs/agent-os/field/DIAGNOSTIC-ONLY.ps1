...
.DESCRIPTION
    ...
    no installs, no service/task modifications.

    Run as Administrator for full CIM/WMI access.
    Output: Desktop\fullsite-diag-<timestamp>\ + ZIP + SHA-256

.NOTES
    Fullsite POS — Field Batch #2 — First NSIS install over legacy deployment.
...
# Startup folders — inventory + resolve .lnk targets (read-only via WScript.Shell)
$startupFolderPaths = @(
...
# PDV3 → SERVER1:7717 connectivity (try candidates; adjust $server1Candidates for the actual LAN)
...
# Collect all unique exe paths from: live processes, folder inventory, uninstall locations
...
foreach ($fi in @($folderInventory)) {
    if ($fi.File -like '*.exe' -and $fi.Exists -and $fi.SHA256 -ne 'SKIPPED-BIOMETRIC') {
...
$exeDetails = $rawExePaths | Sort-Object -Unique | ForEach-Object {
    ...
    $lw   = ''
    try { $lw = (Get-Item $path -ErrorAction SilentlyContinue).LastWriteTime.ToString('yyyy-MM-dd HH:mm:ss') } catch {}
    [PSCustomObject]@{
        Path        = $path
...
# 10. COMPUTE SUMMARY
# =============================================================================
$nsisFound   = (@($uninstallEntries) | Where-Object { $_.UninstallString -like '*\Fullsite POS\*' }).Count -gt 0
$legacyFound = Test-Path 'C:\fullsite' -ErrorAction SilentlyContinue
...
$p7717str = if ($p7717row) { "$($p7717row.ProcessName) PID $($p7717row.OwningProcess) @ $($p7717row.ExecutablePath)" } else { 'NOT BOUND' }
$p7718str = if ($p7718row) { "$($p7718row.ProcessName) PID $($p7718row.OwningProcess) @ $($p7718row.ExecutablePath)" } else { 'NOT BOUND' }

$autoMethods = [System.Collections.Generic.List[string]]::new()
foreach ($re in @($fsRunEntries)) {
    $autoMethods.Add("Registry:Run($($re.RegKey.Split('\')[-1])) name=$($re.Name)")
}
foreach ($si in @($startupItems)) {
    if ($si.LnkTarget -like '*ullsite*') {
        $autoMethods.Add("StartupFolder:$($si.LnkTarget)")
    }
}
