...
- `/pos/corte`: gate de PIN de gerente (sessionStorage `corte_access='1'` para no repetir
  en la misma sesión; cada acceso audita `corte_viewed`). Muestra ventas por método,
  descuentos, propinas del día seleccionado.
...
1. **Corte por fecha, no por turno**: `/pos/corte` filtra `created_at` del día; un turno que
   cruza medianoche se parte. `turno_id` ya se estampa — falta migrar el filtro.
2. **`delivery_orders` paralela**: sin reconciliación con `pos_orders`; el corte no incluye delivery.
   Pendiente SQL de Daniel: `ALTER TABLE delivery_orders ALTER COLUMN id SET DEFAULT gen_random_uuid();`
