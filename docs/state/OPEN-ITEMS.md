...
| OP-40 | Constructor de mapa de mesas: mergear a main + link nav + **unificar coordenadas** | 🟠 P1 | 🔶 **CASI** — #1 mergear ✅ (`plano-editor` ya en main); #3 coords ✅ (`75bfdde7`) `/pos/mesas` + `/pos/plano` ambos leen `x_pct/y_pct/shape` de `pos_mesas` con fallback; #2 link nav ⬜ (editor solo por URL, sin botón — falta botón gerente "Editar mapa"); pend cosmético: `FLOOR_WALLS/FLOOR_LABELS` aún hardcode AMALAY (afecta solo tenants != amalay) | `plano-editor/page.tsx`, `mesas/page.tsx`, `plano/page.tsx` |
