...
### BUG-019 · SIN aislamiento de tenant a nivel DB — anon key pública puede leer/escribir POS de TODOS los tenants · **P0 SECURITY / MARKET BLOCKER** · OPEN
- **Verdicto (inspección READ-ONLY de policies de PRODUCCIÓN `qjiomlvudfmzuvqvhwpk` autorizada por el fundador, 2026-08-06):** **B — PROD VULNERABLE / NO DB-LEVEL TENANT ENFORCEMENT.** Confirmado SOLO inspeccionando policies+grants (NO se leyeron datos cross-tenant de producción).
- **Evidencia (producción):**
  - RLS `enabled=true` en todas las tablas POS — pero las **policies son permisivas**: cada tabla tiene SELECT `using(true)` para `anon`/`public` **sin ninguna referencia a client_id, auth.uid(), auth.jwt() ni app_metadata**.
    - `pos_staff`: `anon_read_staff` SELECT public `using true`
    - `pos_orders`: `anon_read` SELECT + `anon_update_orders` UPDATE + `anon_insert_orders` INSERT, todas `using/check true` para anon
    - `pos_turnos`: `anon_all_turnos` **ALL** (incluye DELETE) para anon `using true`
    - `pos_cash_movements`, `pos_cierres`, `pos_menu_items/categories` (anon UPDATE true), `pos_modifiers`, `pos_recipes`, `pos_audit_log` (anon read+insert): todas anon `using true`
  - Grants: `anon` tiene **SELECT, INSERT, UPDATE, DELETE, TRUNCATE** en pos_staff, pos_orders, pos_turnos, pos_menu_items, pos_cash_movements, pos_cierres, pos_recipes, pos_audit_log.
  - Único scoping correcto en el proyecto: `client_users` (SELECT `auth.uid()=user_id`) y `clients` (SELECT `private.user_has_client_access(id)`). Las tablas POS NO usan ese patrón.
- **Vector de ataque:** la `NEXT_PUBLIC_SUPABASE_ANON_KEY` está embebida en el bundle del navegador (pública por diseño). Con esa key + PostgREST directo (`/rest/v1/pos_orders?client_id=eq.<cualquier-tenant>`), **cualquier persona en internet** (no hace falta login) puede: leer staff/órdenes/pagos/turnos/cortes/movimientos-de-caja/menú/recetas/audit de CUALQUIER tenant; **modificar** órdenes/menús/precios; **borrar** turnos. La protección `withPOSAuth` del servidor es irrelevante — el atacante no usa los endpoints del servidor, va directo a PostgREST con la anon key.
- **Impacto:** fuga total de datos multi-tenant + manipulación/borrado no autorizado. Bloquea absolutamente el onboarding de client #2 y es un riesgo para el tenant actual (AMALAY). Es MARKET BLOCKER.
- **Confirmación adicional (staging):** el mismo modelo reproducido en staging permitió leer 4 staff de `vantara` desde una sesión de `demo`. En prod NO se ejecutó lectura cross-tenant — se probó por policies, como se pidió.
- **Fix preparado (NO aplicado a prod — requiere autorización de Daniel + cambio de app coordinado):** `docs/release/BUG-019-tenant-rls-fix.sql` (migración propuesta). Reemplaza las policies permisivas anon por policies `authenticated` scoped con `private.user_has_client_access(client_id)` (el patrón que ya usa `clients`), revoca grants a `anon` en tablas operativas, y deja `pos_menu_*`/`pos_recipes` legibles sólo por tenant. **Requiere cambio de app**: las lecturas directas del navegador deben usar el JWT de sesión (rol `authenticated`), no la anon key; las lecturas del kiosko POS (PIN→shift-token, sin sesión Supabase) deben enrutarse por endpoints server con `withPOSAuth`. Sin ese cambio de app, aplicar sólo la migración rompe el POS. Test de isolation propuesto: intento de lectura cross-tenant como authenticated de otro tenant → 0 filas.
- **Estado:** OPEN — **P0 MARKET BLOCKER**. Requiere decisión y autorización de Daniel para: (1) aplicar la migración a prod, (2) el cambio de app de anon→authenticated/endpoints. NO aplicar sin autorización.
