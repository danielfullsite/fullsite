...
| E2E-06 Caja (apertura→ventas→movimientos→X→Z→reconciliación) | **PARCIAL (UI) + reconciliado** | Turno abierto fondo $2,000 (DB verificado); Venta A café $60→$69.60 **efectivo** cerrada; Venta B horchata $55→$63.80 + **propina $10** = $73.80 **tarjeta** cerrada. Reconciliación mecánica exacta POS↔`pos_orders`↔`pagos` (cent-level). Depósito/retiro/corte X/corte Z **no completados** — bloqueados por inestabilidad de token del harness (expiración fs-at ~35min sin flujo durable de shift-token; STALE_WRITE por revisión en recargas rápidas). Es límite de harness, NO de producto (prod tiene SHIFT_TOKEN_SECRET → shift-token 8h). Hallazgo: propina POS redondea a peso ($10 vs $9.57 calculado) — decisión de negocio, no bug. | Depósito/retiro/corte UI (re-ejecutar con shift-token estable) |
...
| E2E-08 Tenant isolation | **FAIL — ver BUG-019 (P0)** | Twin cross-tenant probe REJECT a nivel WS (`restaurant_id mismatch`) sigue PASS; PERO la inspección read-only de policies de PRODUCCIÓN reveló que el aislamiento DB NO existe: anon key pública lee/escribe POS de todos los tenants vía PostgREST. MARKET BLOCKER. | Fix RLS tenant-scoped + cambio app anon→authenticated (autorización Daniel) |
