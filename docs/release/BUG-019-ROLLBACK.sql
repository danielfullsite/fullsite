...
-- Solo funciones QUE POSEEMOS (owner = current_user): funciones de extensiones
-- (p.ej. dblink_connect_u) las posee un superusuario y no se pueden grantear —
-- además el fix nunca les tocó permisos. Best-effort per-función: si un grant
-- falla, se omite y el rollback continúa (no debe abortar por una función ajena).
do $$
declare r record;
begin
  ...
  loop
    ...
    exception when others then
      raise notice 'rollback: skip grant %(%): %', r.proname, r.args, sqlerrm;
    end;
  end loop;
end $$;
